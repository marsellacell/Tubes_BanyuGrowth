...
<head>
  ...
  <title>Produk Saya - BanyuGrowth</title>
      ...
      <div class="max-w-7xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
          <div>
            ...
            <p class="text-gray-600 mt-1">Kelola semua produk yang Anda jual</p>
          </div>
          <a href="/umkm/products/create" class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg transition font-semibold shadow-lg">
        ...
        <div class="bg-white rounded-xl shadow-lg p-12 text-center">
          ...
          <p class="text-gray-500 mb-6">Mulai tambahkan produk pertama Anda untuk ditampilkan di marketplace</p>
          <a href="/umkm/products/create" class="inline-block bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg font-semibold transition shadow-lg">
            <i class="bi bi-plus-circle mr-2"></i>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold text-purple-600">{{ number_format($products->sum('jumlah_view')) }}</p>
              </div>
          ...
          <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm">Total Klik Beli</p>
                <p class="text-2xl font-bold text-orange-600">{{ number_format($products->sum('jumlah_klik_beli')) }}</p>
              </div>
                ...
                <tr class="hover:bg-gray-50 transition">
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-4">
                      @if($product->gambar)
                      <img src="{{ Storage::url($product->gambar) }}" alt="{{ $product->nama_produk }}" class="w-16 h-16 object-cover rounded-lg shadow">
                      @else
                      ...
                      <div>
                        ...
                        <div class="text-sm text-gray-500 mt-1">{{ Str::limit($product->deskripsi, 60) }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                    <span class="px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-700">
                      {{ $product->category->nama_kategori ?? 'Lainnya' }}
                    </span>
                  </td>
                  <td class="px-6 py-4">
